...
- For every task that changed since the prior save, the manifest must include a concise summary of material updates rather than a word-for-word transcript. On load, resolve each changed task through its binding and dispatch exactly one concise checkpoint update to that task; do not message unchanged or unresolved tasks. The agent performs summary and dispatch work, while `tools/checkpoint.ps1` validates the manifest, publishes saves, and emits the deterministic load plan. The Codex app controls any visual update indicator; do not claim a guaranteed blue-dot count.
