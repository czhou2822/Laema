...
- Stage 1–6 use the declarative sequence evaluator; Stage 7 and Stage 8 use the configured Heat threshold and Heat-gated sequence evaluators.
- Stages 1–8 use independently instantiated shared training-area content with one stationary, non-attacking, refillable dummy per stage.
...
- Stage 1–8 refill targets plus the extracted Final Arena's three permanent pass-through practice targets and distinct solid, killable Final Enemy;
...
- Stage 3 presentation instructing `Hold R2 + X -> X -> X -> X -> X -> Release R2` through compact objective tokens while retaining its existing five-X-plus-level-5-Cast evaluator;
- reusable Stage Area scenes, StageDirector lifecycle, Stage 1–8 evaluators plus final-enemy evaluator, transition camera flow, stage reset, and final free-practice presentation;
...
- scenario-level results for each Stage 1–8 objective and transition, because the user supplied only a broad earlier report;
