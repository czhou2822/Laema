...
func apply(
	...
	earth_config: Dictionary,
	direct_damage_multiplier: float = 1.0
) -> void:
	if level < 1 or world == null:
		return
	var level_data: Dictionary = earth_config["levels"][level - 1]
	var radius := (
		float(earth_config["base_radius"])
		+ float(earth_config["radius_per_level"]) * float(level - 1)
	)
